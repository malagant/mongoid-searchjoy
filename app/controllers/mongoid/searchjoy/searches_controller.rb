module Mongoid
  module Searchjoy
    class SearchesController < ActionController::Base
      layout 'mongoid/searchjoy/application'
      ...
      def overview
        relation = Mongoid::Searchjoy::Search.where(search_type: params[:search_type])
        @searches_by_week = relation.group_by_week(:created_at, Time.zone, @time_range).count
        @conversions_by_week = relation.where(:converted_at.exists => true).group_by_week(:created_at, Time.zone, @time_range).count
      ...
      def recent
        @searches = Monogid::Searchjoy::Search.order_by(created_at: 1).limit(50)
        render layout: false
      end
      ...
      def set_time_zone
        @time_zone = Mongoid::Searchjoy.time_zone || Time.zone
      end
